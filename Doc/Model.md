...
Agent 和 Map 检测任务都包含“无”类别，并使用匈牙利匹配。检测头和检测监督使用前 12 层逐层旁路残差累积后的检测查询（第 12 层 `Acc_{11}`，不含身份嵌入）；第 13 层之后的检测 Token 只作为后续规划上下文参与主干计算，不再接检测监督。骨干中间（Transformer、逐层残差投影、逐层身份嵌入）使用 BF16，仅初始检测查询与检测解码器保持 FP32。
