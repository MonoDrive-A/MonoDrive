...
5. 主干网络包含 16 层 Transformer，其中第 1-12 层不输入目标导航点，第 13 层输入前加入目标导航点 Token。
6. 模型只使用前向单目图像。
...
9. 在第 $Index=2, 4, 6, 8, 10$ 层使用模态独立 FFN：
   ...
   - 驾驶相关 Token：检测查询、轨迹查询；
   - 两类 Token 分别进入独立的 FFN 分支。
10. 第 13-16 层使用单路 FFN，不再使用模态独立 FFN。

### 4. 序列组织、感知和规划

#### 4.1 Token 序列

模型统一序列最终由以下部分组成，总长度为 2614。第 1-12 层输入不包含目标导航点 Token，长度为 2612；第 13 层输入前再追加目标导航点 Token。
...
Agent 和 Map 检测任务都包含“无”类别，并使用匈牙利匹配。检测头和检测监督使用前 12 层逐层旁路残差累积后的检测查询（第 12 层 `Acc_{11}`，不含身份嵌入）；每层残差投影前先经逐层 RMSNorm。第 13 层之后的检测 Token 只作为后续规划上下文参与主干计算，不再接检测监督。骨干中间（Transformer、逐层残差 RMSNorm、逐层残差投影、逐层身份嵌入）使用 BF16，仅初始检测查询与检测解码器保持 FP32。
...
轨迹词表概率和轨迹残差由第 16 层最终输出的轨迹 Token 解码，并作为最后一层唯一直接监督。
...
当前训练数据处理不启用危险轨迹屏蔽。H5 中未来 Agent 标注较稀疏，若用不完整未来框判断碰撞，会错误压低本应保留的候选轨迹。因此轨迹词表 soft label 只由词表轨迹与 GT 轨迹之间的物理空间 MSE 构造。
...
其中当前 ego 原点作为 $p_0$ 参与第一个未来点平滑，最后一个 3 秒端点保持不变。平滑不能改变轨迹采样频率、点数或坐标系；开启后必须记录实验口径，并用可视化检查目标点和未来轨迹是否已经偏离真实行为。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-08 | 1os3_Codex | AI 完成：将主干改为 16 层，第 1-12 层不含目标点，第 13 层输入前加入目标点 Token；检测监督改用第 12 层输出，最终层只监督轨迹词表概率和残差；检测查询改为 Agent 16 个、Map 32 个。 |
| 2026-06-07 | 1os3_Codex | AI 完成：明确检测查询不按类别硬分配，补充 Agent 4 个 mode 在 120 度空间内均匀初始化，并声明检测解码线性层使用 FP32 且不做反 Symlog。 |
| 2026-06-07 | 1os3_Codex | AI 完成：移除训练阶段危险轨迹判断要求，轨迹词表 soft label 改为只由物理空间 MSE 构造，以适配稀疏 H5 未来 Agent 标注。 |
| 2026-06-07 | 1os3_Codex | AI 完成：将 Symlog 与尺寸对数公式中的 `Log/log` 记法修正为自然对数 `ln`，数值口径不变。 |
