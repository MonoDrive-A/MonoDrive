...
| `close_loop/monodrive/h5_export.py` | 把闭环推理输入写成 `b2d_h5_v5` H5，供开环 `B2DH5Dataset` 与 backbone PCA 可视化读取。 | [close_loop/monodrive/h5_export_abstract.md](close_loop/monodrive/h5_export_abstract.md) | [close_loop/monodrive/h5_export.md](close_loop/monodrive/h5_export.md) | 已同步 |
...
| `config/backbone.toml` | 保存统一序列 Transformer 主干配置，包括子配置路径、16 层主干、视觉 Token 专用 3D RoPE、模态独立 FFN 和精度策略。 | [config/backbone_abstract.md](config/backbone_abstract.md) | [config/backbone.md](config/backbone.md) | 已同步 |
| `config/detection_head.toml` | 保存检测查询初始化和检测解码头配置，包括 Agent 16 / Map 32 检测查询、无类别硬分配、Agent 4-mode 120 度均匀初始化和 FP32 解码精度。 | [config/detection_head_abstract.md](config/detection_head_abstract.md) | [config/detection_head.md](config/detection_head.md) | 已同步 |
...
| `config/trajectory_vocab.toml` | 保存模型侧轨迹词表加载、嵌入和解码配置，包括归一化词表字段、384 维特征、解码前 RMSNorm eps 和解码初始化。 | [config/trajectory_vocab_abstract.md](config/trajectory_vocab_abstract.md) | [config/trajectory_vocab.md](config/trajectory_vocab.md) | 已同步 |
...
| `model/backbone.py` | 复用已有嵌入与解码模块，构造 16 层两阶段统一序列主干；前 12 层检测 Token 逐层旁路残差（含逐层 RMSNorm + 零初始化投影）与 per-layer 身份嵌入，第 12 层 Acc 接检测头，第 13 层前注入目标点 Token。 | [model/backbone_abstract.md](model/backbone_abstract.md) | [model/backbone.md](model/backbone.md) | 已同步 |
| `model/detection_head.py` | 用可学习 symlog 坐标经线性层编码无类别硬分配的 Agent/Map 检测查询 Token，并用 FP32 线性层以 reference + offset 解码检测输出。 | [model/detection_head_abstract.md](model/detection_head_abstract.md) | [model/detection_head.md](model/detection_head.md) | 已同步 |
...
| `model/trajectory_vocab/trajectory_vocab.py` | 从 TOML 配置加载归一化轨迹词表，生成 384 维轨迹查询，并在解码前经 TokenRMSNorm 后输出轨迹 logit 与 Tanh 残差。 | [model/trajectory_vocab/trajectory_vocab_abstract.md](model/trajectory_vocab/trajectory_vocab_abstract.md) | [model/trajectory_vocab/trajectory_vocab.md](model/trajectory_vocab/trajectory_vocab.md) | 已同步 |
...
| `train/checkpointing.py` | 保存和加载训练 checkpoint，记录 model、optimizer、scheduler、step、epoch、batch、metrics 和 RNG 状态，并兼容恢复被加载到训练设备上的 RNG byte tensor。 | [train/checkpointing_abstract.md](train/checkpointing_abstract.md) | [train/checkpointing.md](train/checkpointing.md) | 已同步 |
...
| `train/trainer.py` | 运行完整训练流程，包含模型、数据、loss、优化器、学习率调度、梯度监测、自动保存和 sampler 级断点恢复。 | [train/trainer_abstract.md](train/trainer_abstract.md) | [train/trainer.md](train/trainer.md) | 已同步 |
...
| `visualization/backbone_feature_pca_viewer.py` | 直接调用 16 层统一主干并以 FP32 导出每层视觉 Token PCA 图，支持加载 checkpoint 权重，同时展示非 `none` 检测查询、轨迹词表概率和 top-k residual 修正，并默认额外保存可按 `p(none)` 截断的检测 BEV PNG；支持 `--mask-vision-embeddings`、`--target-x`/`--target-y` 与 `--zero-speed` 做前向输入消融诊断。 | [visualization/backbone_feature_pca_viewer_abstract.md](visualization/backbone_feature_pca_viewer_abstract.md) | [visualization/backbone_feature_pca_viewer.md](visualization/backbone_feature_pca_viewer.md) | 已同步 |
