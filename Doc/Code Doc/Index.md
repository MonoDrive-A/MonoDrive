...
| `visualization/backbone_feature_pca_viewer.py` | 直接调用 16 层统一主干并以 FP32 导出每层视觉 Token PCA 图，支持加载 checkpoint 权重，同时展示非 `none` 检测查询、轨迹词表概率和 top-k residual 修正，并默认额外保存可按 `p(none)` 截断的检测 BEV PNG。 | [visualization/backbone_feature_pca_viewer_abstract.md](visualization/backbone_feature_pca_viewer_abstract.md) | [visualization/backbone_feature_pca_viewer.md](visualization/backbone_feature_pca_viewer.md) | 已同步 |
