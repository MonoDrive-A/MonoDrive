...
| `visualization/backbone_feature_pca_viewer.py` | 直接调用统一主干并以 FP32 导出每层视觉 Token PCA 图，支持加载 checkpoint 权重，同时在 BEV 面板展示最高概率类别非 `none` 的 Agent 和 Map 查询。 | [visualization/backbone_feature_pca_viewer_abstract.md](visualization/backbone_feature_pca_viewer_abstract.md) | [visualization/backbone_feature_pca_viewer.md](visualization/backbone_feature_pca_viewer.md) | 已同步 |
