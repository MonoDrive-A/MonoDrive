...
`config/target_point_embedding.toml` 保存目标点嵌入层的可提交配置：输入目标点维度、`18x16` ego 栅格覆盖范围、三层卷积下采样结构、展平后线性投影为 2 个目标导航点 Token 的输出口径，以及强制 FP32 精度。
...
| 栅格向量场 | `[B, 18, 16, 2]` | 当前为 `grid_xy - target_point`。 |
| 卷积输入 | `[B, 2, 18, 16]` | channel-first 向量场。 |
