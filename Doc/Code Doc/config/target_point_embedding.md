...
`config/target_point_embedding.toml` 集中保存目标点嵌入层配置，包括目标点坐标维度、栅格覆盖范围、三层卷积结构、下采样输出尺寸、线性投影输出 Token 数和强制 FP32 精度。
...
| 目标点向量场 | `[B, 18, 16, 2]` | 当前配置为 `grid_xy - target_point`。 |
| 卷积输入 | `[B, 2, 18, 16]` | 向量场转为 channel-first。 |
...
配置读取端要求所有表和字段显式存在。`[grid]` 中的范围用于生成栅格中心坐标；当前 `vector_order = "grid_minus_target"` 表示目标点到每个栅格中心的相对偏移按 `grid_xy - target_point` 计算。
