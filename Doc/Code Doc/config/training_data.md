...
| Agent 匹配目标 | `[B, 48]`、`[B, 48, 11]` | 分类、状态、mode 和 future 监督。 |
| Map 匹配目标 | `[B, 48]`、`[B, 48, 100, 2]` | 分类和局部地图点监督。 |

## 5. 关键实现逻辑

训练数据配置把 H5 路径解析限制在项目目录内。`dataset.h5_dir` 指向逐场景 H5 目录，加载时展开当前目录下的 `*.h5`。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `dataset.h5_dir` | `data/preprocessed` | 逐场景 H5 目录。 |
