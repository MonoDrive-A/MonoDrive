...
| `[query_embedding]` | TOML table | 查询可学习坐标的初始化映射（symlog 空间）。 |
...
- 功能：声明查询可学习 2D 坐标参数所在空间的初始化映射。
- 输入：由 Agent / Map 空间采样得到的 ego XY 物理 anchor。
- 输出：经 `coordinate_transform`（symlog）映射的初始 symlog 空间坐标，喂给查询线性编码器。
- 说明：`coordinate_transform` 只在初始化时使用一次；前向直接用线性层编码 symlog 坐标，全程不做反 symlog。
...
查询 Token 由可学习 2D 坐标编码得到：物理 anchor 经 `[query_embedding].coordinate_transform`（symlog）映射为可学习的 symlog 空间坐标参数，再由线性层编码为 `hidden_dim` 维查询。梯度直接更新 symlog 坐标，前向与解码全程不做反 symlog。

Agent 解码采用 reference + offset：`x/y` 直接叠加查询 symlog 坐标，`sin_yaw/cos_yaw` 叠加 symlog 坐标方向 `atan2(symlog_y, symlog_x)` 的 sin/cos；位置和朝向对应解码行零初始化，使初始输出等于参考。尺寸按 `log1p` 物理长宽高写入 bias，速度和加速度按 Symlog 写入 bias。4 个 mode 的 future bias 由配置角度和距离生成，每个未来点按 ego 坐标 `[x, y] = [d cos(theta), d sin(theta)]` 构造，再做 Symlog 变换。

Map 解码同样采用 reference + offset：每条 Map 元素的全部点初始叠加其查询 symlog 坐标，使初始点位于对应空间 anchor；点解码行零初始化。Map 查询不绑定具体地图类别。
...
| `query_embedding.coordinate_transform` | `symlog` | 把物理 anchor 映射到可学习坐标空间的初始化变换。 |
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-13 | 1os3 | AI 完成：`[query_embedding]` 改为 `coordinate_transform`，查询改为可学习 symlog 坐标经线性层编码，解码改 reference + offset。 |
