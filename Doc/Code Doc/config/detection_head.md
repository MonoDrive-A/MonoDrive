...
`config/detection_head.toml` 集中保存模型侧检测查询初始化和检测解码头配置。它定义 96 个检测查询的组织方式、查询 Token 初始 anchor 特征、Agent / Map 输出字段、Agent 4 个 future mode 的 120 度均匀初始化、解码线性层初始 logit 和强制 FP32 精度。
...
- Shape：Agent 查询 48 个、Map 查询 48 个，总计 96 个，每个 384 维。
...
- Shape：类别为 3 个前景类加“无”类别；状态 11 维；future 为 `[48, 4, 6, 2]`。
...
- 输出：Map 类别和局部 Map 点。
- Shape：类别为 3 个前景类加“无”类别；点输出为 `[48, 100, 2]`。

## 4. 输入输出与 Shape

| 名称 | Shape | 说明 |
| --- | --- | --- |
| 检测查询 Token | `[96, 384]` | 前 48 个为 Agent，后 48 个为 Map。 |
| Agent anchor | `[48, 2]` | ego 坐标系空间 anchor，前方 120 度内均匀采样。 |
| Map anchor | `[48, 2]` | ego 坐标系空间 anchor，前方 120 度内均匀采样。 |
| Agent class logits | `[B, 48, 4]` | `car/bicycle/pedestrian/none` 未激活 logit。 |
| Agent states | `[B, 48, 11]` | `[x, y, length_log1p, width_log1p, height_log1p, sin_yaw, cos_yaw, vx, vy, ax, ay]`。 |
| Agent mode logits | `[B, 48, 4]` | 4 个 Agent future mode 的未激活 logit。 |
| Agent future | `[B, 48, 4, 6, 2]` | Symlog 空间 future 位移预测，不做反 Symlog。 |
| Map class logits | `[B, 48, 4]` | `lane_divider/road_edge/centerline/none` 未激活 logit。 |
| Map points | `[B, 48, 100, 2]` | Symlog 空间 ego XY Map 点预测，不做反 Symlog。 |

## 5. 关键实现逻辑

Agent 查询和 Map 查询都不按类别做硬性数量分配。查询初始化只按空间 anchor 均匀覆盖前方 120 度区域；类别预测完全由解码线性层输出的分类 logit 决定。

Agent / Map 空间 anchor 由 `angle_min_deg`、`angle_max_deg`、`radial_min_m`、`radial_max_m`、`radial_count`、`angle_count` 和 `spatial_order` 决定。当前 `6 * 8 = 48` 个 anchor 正好对应每个任务的 48 个查询。
...
| `query.agent_query_count` | `48` | Agent 查询数量，不按类别硬分配。 |
| `query.map_query_count` | `48` | Map 查询数量，不按类别硬分配。 |
