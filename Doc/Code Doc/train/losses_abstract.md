...
传入 `MonoDriveBackboneOutput` 和 `TrainingBatchLabels`。轨迹词表分数在 loss 内部使用 `log_softmax` 计算 soft CE；Agent / Map 分类 CE 传入 raw logits，并按 `detection_class_weights.mode` 选择 `auto`（分组 Focal + sqrt 背景缩放）、`matched`（分组 Focal + none 下采样配对）、`manual` 或 `disabled`。`matched` 在 non-none 全量监督的同时，仅在 `N_none > N_fg` 时对 none 无放回随机下采样至 `N_fg`，两组均值直接相加；mode CE 只监督存在有效 future 点的匹配 query。训练日志会额外输出 Agent / Map 分类 CE 的 none 与 non-none 分项，便于分别观察匹配与未匹配 query 的学习情况。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-09 | 1os3_Composer | AI 完成：同步 `matched` 检测分类 none 下采样配对模式摘要。 |
