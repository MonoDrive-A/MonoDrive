...
Agent / Map 分类使用 hard-label CE，并在 none / non-none 两组之间应用检测分类权重。`disabled` 模式等价于未加类别权重的 batch mean CE；`manual` 模式使用配置中的 `*_non_none_weight` 和 `*_none_weight` 做加权 CE；`auto` 模式使用匹配 / 未匹配分离的分组 Focal Loss：匹配 query 在前景类上竞争并额外用 objectness BCE（`logsumexp(fg) - none`）压低 none logit，未匹配 query 仍监督 none；两组分别求 Focal 均值，背景组按 `sqrt(N_fg / N_bg)` 自动缩放（clamp 到 `[0.08, 1.0]`），focal gamma 按组内目标置信度在 `[1.0, 4.0]` 内自适应。Map 分类在 `auto` 模式下还会按 `sqrt(map_point_count * point_dim / foreground_class_count)` 自动放大 loss，以抵消 100 点回归对共享 Map 特征的梯度压制。
...
| `detection_class_weights.mode` | `auto` | Agent / Map 分类 CE 策略，支持 `auto`（分组 Focal）、`manual`、`disabled`。 |
...
- 自动检测分类平衡使用匹配 / 未匹配分离的分组 Focal Loss；Map `auto` 分类 loss 还会按点回归维度自动放大。
