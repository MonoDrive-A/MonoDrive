...
Agent / Map 分类使用 hard-label CE，并在 none / non-none 两组之间应用检测分类权重。`disabled` 模式等价于未加类别权重；`manual` 模式使用配置中的 `*_non_none_weight` 和 `*_none_weight`；`auto` 模式按当前 batch 的目标数量计算权重，使 none 与 non-none 两组在分类 CE 中的总权重接近相等，并用 `auto_min_weight` / `auto_max_weight` 限制极端值。若当前 batch 只有 none 或只有 non-none 目标，则自动模式退回未加类别权重的 CE。
...
- 自动检测分类权重只使用当前 batch 中已构造的分类目标，不读取或写入额外统计文件。
