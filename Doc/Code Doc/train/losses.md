...
Agent / Map 分类使用 hard-label CE，并在 none / non-none 两组之间应用检测分类权重。`disabled` 模式等价于未加类别权重的 batch mean CE；`manual` 模式使用配置中的 `*_non_none_weight` 和 `*_none_weight` 做加权 CE；`auto` 模式使用匹配 / 未匹配分离的分组全类 Focal Loss：匹配 query 与未匹配 query 均在完整 softmax 上监督硬标签（前者目标为前景类，后者目标为 none），两组分别求 Focal 均值后乘以 `*_non_none_weight` / `*_none_weight`，背景组再按 `sqrt(N_fg / N_bg)` 自动缩放（clamp 到 `[0.08, 1.0]`），focal gamma 按组内目标置信度在 `[1.0, 4.0]` 内自适应；`matched` 模式同样使用分组全类 Focal Loss，但 non-none 始终全量监督，none 仅在 `N_none > N_fg` 时无放回随机下采样至 `N_fg`，两组均值直接相加、不再缩放背景组。Map 分类在 `auto` / `matched` 模式下还会按 `sqrt(map_point_count * point_dim / foreground_class_count)` 自动放大 loss，以抵消 100 点回归对共享 Map 特征的梯度压制。
...
训练日志会额外记录 Agent / Map 检测分类 CE 的 none 与 non-none 分项（`agent_class_ce_non_none`、`agent_class_ce_none`、`map_class_ce_non_none`、`map_class_ce_none`），不参与总 loss 加权，仅用于监控。`auto` 模式下 non-none 为匹配 query 的全类 Focal 并乘以 `*_non_none_weight`，`none` 为未匹配 query 的全类 Focal、乘以 `*_none_weight` 后再按 `sqrt(N_fg / N_bg)` 缩放；`matched` 模式下分项为两组未缩放的 Focal 均值；Map `auto` / `matched` 分项同样包含点回归维度 auto scale。
...
| `detection_class_weights.mode` | `auto` | Agent / Map 分类 CE 策略，支持 `auto`（分组 Focal + sqrt 缩放）、`matched`（分组 Focal + none 下采样配对）、`manual`、`disabled`。 |
| `detection_class_weights.agent_non_none_weight` | 见 `config/training.toml` | `manual` 为 Agent 前景类权重；`auto` / `matched` 为匹配 query 组 loss 乘子。 |
| `detection_class_weights.agent_none_weight` | 见 `config/training.toml` | `manual` 为 Agent none 类权重；`auto` / `matched` 为未匹配 query 组 loss 乘子。 |
| `detection_class_weights.map_non_none_weight` | 见 `config/training.toml` | `manual` 为 Map 前景类权重；`auto` / `matched` 为匹配 query 组 loss 乘子。 |
| `detection_class_weights.map_none_weight` | 见 `config/training.toml` | `manual` 为 Map none 类权重；`auto` / `matched` 为未匹配 query 组 loss 乘子。 |
...
- 自动检测分类平衡使用匹配 / 未匹配分离的全类 Focal Loss，两组分别乘以 `*_non_none_weight` / `*_none_weight`；Map `auto` 分类 loss 还会按点回归维度自动放大。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-09 | 1os3_Composer | AI 完成：新增 `matched` 检测分类模式，none 在多于 non-none 时下采样配对，两组均值直接相加。 |
| 2026-06-09 | 1os3_Composer | AI 完成：`auto` 检测分类改为匹配 / 未匹配分组的全类 Focal Loss，移除前景 objectness，两组分别乘以 `*_non_none_weight` / `*_none_weight`。 |
