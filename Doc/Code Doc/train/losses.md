...
训练日志会额外记录 Agent / Map 检测分类 CE 的 none 与 non-none 分项（`agent_class_ce_non_none`、`agent_class_ce_none`、`map_class_ce_non_none`、`map_class_ce_none`），不参与总 loss 加权，仅用于监控。分项为对应 query 子集上的标准 Focal 均值。
...
- 自动检测分类平衡使用标准 Focal Loss；Map 分类与 Agent 使用同一 Focal 口径，不再额外放大。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-10 | 1os3_Composer | AI 完成：移除 Map `auto` 分类 loss 的点回归维度 auto scale，避免梯度放大。 |
