...
- 自动检测分类平衡使用匹配 / 未匹配分离的分组 Focal Loss；匹配 query 在前景类上竞争并额外监督 objectness，背景组缩放与 focal gamma 均按组内统计自适应。
