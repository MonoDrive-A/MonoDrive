...
当前 ego 原点作为第一个未来点的前置锚点，最后一个 3 秒端点保持不变。该平滑不改变 2Hz 点数、未来帧索引或坐标系，但可能改变轨迹形状和目标候选分布；开启后必须记录实验口径。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-07 | 1os3_Codex | AI 完成：移除预处理文档中与危险轨迹碰撞判定绑定的平滑口径，保持与训练数据处理当前不启用危险轨迹判断一致。 |
