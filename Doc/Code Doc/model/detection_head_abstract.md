...
`model/detection_head.py` 实现模型侧检测查询初始化和检测解码头。它读取 `config/detection_head.toml`，生成 Agent / Map 共 96 个检测查询 Token，并用 FP32 线性层从 `[B, 96, 384]` 检测 Token 特征解码 Agent 和 Map 输出。
...
| `DetectionQueryEmbedding.forward` | 无 | `[96, 384]` |
| `DetectionHeadDecoder.forward` | `[B, 96, 384]` | `DetectionDecoderOutput` |
| `agent_class_logits` | - | `[B, 48, 4]` |
| `agent_states` | - | `[B, 48, 11]` |
| `agent_future_trajectories` | - | `[B, 48, 4, 6, 2]` |
| `map_points` | - | `[B, 48, 100, 2]` |
...
| `DetectionHeadDecoder` | 输入必须是浮点 `[B, 96, 384]`；内部禁用 autocast 并输出 FP32。 |
