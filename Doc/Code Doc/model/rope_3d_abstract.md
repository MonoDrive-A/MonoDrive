...
提供通用 3D RoPE 旋转位置编码。模块只消费调用方传入的三维位置坐标、轴通道划分和 RoPE 基频，对特征张量最后一维的前若干通道执行三轴旋转，并强制输出 FP32。
...
| 接口 | 使用规范 |
| --- | --- |
| `apply_rope_3d(features, positions, axis_dims, theta)` | `axis_dims` 必须包含 3 个正偶数，且总和不能超过 `features.shape[-1]`；输出固定为 FP32。 |
...
- 该模块强制输出 FP32；下游若需要 BF16/FP16，必须在调用方显式转换。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-07 | 1os3_Codex | AI 完成：记录 3D RoPE 强制 FP32 输出约定。 |
